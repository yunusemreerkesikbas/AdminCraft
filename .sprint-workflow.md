...
#### Sprint 12: Platform DB + Database-Per-Tenant Integration (1 week)

Goal: Introduce control-plane Platform DB and per-tenant databases with DATABASE multi-tenancy; enable provisioning from Admin (`tenants-list`) with module selection and Flyway-based migrations.

Scope (Backend):

- Platform DB schema (control plane):
  - `platform.tenants(id, subdomain, custom_domain?, db_host, db_port, db_name, status, default_language, supported_languages, created_at, updated_at)`
  - `platform.modules_catalog(id, code UNIQUE, name, type(core|b2b|b2c), version, deps JSON, enabled_by_default)`
  - `platform.tenant_modules(id, tenant_id, module_code, status(enabled|disabled|pending), target_version)`
  - `platform.provisioning_jobs(id, tenant_id, type(create-db|add-modules|migrate), payload JSON, status, progress, error?, created_at, updated_at)`
- Multi-tenancy wiring:
  - `MultiTenantConnectionProvider` (Hikari DataSource cache + LRU idle close)
  - `CurrentTenantIdentifierResolver` (resolves tenant DB name from `TenantContext`)
  - Request filter: resolve tenant from subdomain/JWT → set/clear `TenantContext` (try/finally)
- Provisioning service:
  - Create DB (`CREATE DATABASE <db_name> ...`)
  - Programmatic Flyway migrate with module-based locations (core + selected modules)
  - Minimal seed (site settings baseline, roles)
  - Job tracking: status/progress/logs
- Flyway strategy:
  - Platform DB locations: `db/platform`
  - Tenant DB core: `db/tenant/core`
  - Modules: `db/tenant/<moduleCode>` (e.g., `pagebuilder`, `site_settings`, `b2c_orders`, `b2b_quotes`)
  - Convert `schema-page-builder.sql` into versioned Flyway scripts under the relevant module (baseline V1__...)
  - Idempotent seeds via repeatable migrations (R__*.sql)

Scope (Frontend Admin):

- `tenants-list` → “Provision DB / Modules” dialog
  - Multi-select modules (grouped: core/b2b/b2c), dependency-aware selection
  - Start job; show progress bar, status badges; simple retry on failure
- Job list/status view (minimal): last N jobs for a tenant with timestamps

Public API (initial):

- `POST /api/tenants/{id}/provision` → body `{ modules: string[] }` → returns `jobId`
- `GET /api/provisioning/jobs/{jobId}` → `{ status, progress, error?, startedAt, finishedAt }`
- (Optional) `GET /api/modules/catalog` → list selectable modules for dialog

Refactors/Removals (breaking where noted):

- Controllers/Services: remove `tenantId` parameters from signatures; tenant comes from `TenantContext` (BREAKING)
- Repositories: drop explicit `tenantId` filters (kept temporarily if you retain `tenant_id` columns during transition)
- Disable `spring.jpa.hibernate.ddl-auto` for tenant DBs; rely on Flyway
- Deprecate `X-Tenant-ID` header usage; prefer subdomain/JWT resolution (keep as fallback during transition)
- Migrate `schema-page-builder.sql` into Flyway; stop executing manual SQL files
- Docker init script: create only Platform DB; tenant DBs created by provisioning

Acceptance Criteria:

- From Admin, starting a provisioning job with selected modules creates a new tenant DB, applies core + selected module migrations, and seeds minimal data
- Two tenants operate on isolated DBs; data from tenant A is not visible in tenant B
- Flyway migrations are re-runnable; provisioning jobs are idempotent and report progress
- Requests without a resolvable tenant are rejected with a localized 400/401

Risks & Mitigations:

- DS explosion: cap Hikari pool per-tenant; LRU idle eviction
- Migration drift: module-based Flyway locations; platform keeps desired state
- Wrong-tenant leakage: request filter with try/finally clear; remove header reliance

Workflow:

1) Initialize migrations
   - Add Flyway dependency and base config
   - Create `db/platform` baseline (tenants, modules_catalog, tenant_modules, provisioning_jobs)
2) Platform datasource & repos
   - Configure `platformDataSource` and JPA context
   - Implement repositories/services for platform tables
3) Module catalog seed
   - Seed core/b2b/b2c modules (code, deps, enabled_by_default)
4) Tenant resolution
   - Implement request filter to resolve tenant (subdomain/JWT) and set/clear `TenantContext`
5) DATABASE multi-tenancy
   - Implement `MultiTenantConnectionProvider` with Hikari DS cache (LRU idle eviction)
   - Implement `CurrentTenantIdentifierResolver` (dbName from context)
6) Tenant migrations layout
   - Create `db/tenant/core` baseline
   - Move Page Builder schema into `db/tenant/pagebuilder` as versioned migrations
   - Remove manual `schema-page-builder.sql` execution
7) Provisioning service
   - `createDatabaseIfNotExists(dbName)`
   - `runMigrations(dbName, modules[])` (core + selected modules)
   - Minimal seed (roles/site settings); job status/progress logging
8) REST APIs
   - `POST /api/tenants/{id}/provision` (modules[] → jobId)
   - `GET /api/provisioning/jobs/{jobId}` (status/progress)
   - `GET /api/modules/catalog` (optional)
9) Admin UI
   - Add “Provision DB / Modules” dialog to `tenants-list`
   - Fetch module catalog; dependency-aware selection; start job; show progress
10) Isolation tests

- Spin up two tenants; CRUD in A not visible in B
- Re-run provisioning idempotency

11) Refactors & hardening

- Disable `ddl-auto` for tenant DBs; rely on Flyway
- Deprecate `X-Tenant-ID` header; prefer subdomain/JWT (keep as fallback during transition)
- Begin removing `tenantId` parameters from service/controller signatures (phased)

12) Docs & ops

- Update README/CLAUDE with setup
- Add rollback/retry notes for provisioning jobs
