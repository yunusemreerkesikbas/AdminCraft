...
public class PageCategoryServiceImpl implements PageCategoryService {
  ...
  public PageCategory update(PageCategory category) {
    ...
    Long newParentId = category.getParentId() != null ? category.getParentId() : existing.getParentId();
    boolean parentChanged = !Objects.equals(existing.getParentId(), newParentId);
    ...
    existing.setStatus(category.getStatus() != null ? category.getStatus() : existing.getStatus());
  ...
  public void move(Long tenantId, Long categoryId, Long newParentId) {
    ...
    // ÇÖZÜM: Path ve level calculation'ı Domain layer'a taşınmalı (TODO: Business
    // logic refactor)
   ...
   * SERIALIZABLE isolation level + path-based validation önler race
   * condition'ları.
   */
  ...
  private List<PageCategoryDto> toDtoLocalizedBatch(List<PageCategory> categories, Language lang, Long tenantId) {
    ...
    List<com.backend.domain.entity.PageCategoryTranslation> translations = translationRepository
        .findByTenantIdAndCategoryIdInAndLanguage(tenantId, categoryIds, lang);
