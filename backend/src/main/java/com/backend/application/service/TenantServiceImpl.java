...
public class TenantServiceImpl implements TenantService {
  ...
  public TenantServiceImpl(
      ...
      @org.springframework.beans.factory.annotation.Qualifier("tenantModuleDomainRepository") TenantModuleRepository tenantModuleRepository,
      ProvisioningJobRepository provisioningJobRepository,
      TenantContextPort tenantContext,
      FrontendConfigPort frontendConfig) {
  ...
  public List<TenantModuleResponse> getTenantModules(Long tenantId, Language displayLanguage) {
    ...
    return withPlatformContext(() -> {
      ...
      List<TenantModule> tenantModules = getEnabledUserFacingModules(tenantId);
      ...
      return tenantModules.stream()
          .map(tm -> TenantModuleResponse.builder()
              ...
              .moduleName(
                  tm.getModuleCatalogName() != null ? tm.getModuleCatalogName()
                      : tm.getModuleCode())
              .status(tm.getStatus())
  ...
  private Integer countProvisionedModules(Long tenantId) {
    return getEnabledUserFacingModules(tenantId).size();
  }
  ...
  private TenantMetrics loadTenantMetrics(List<Long> tenantIds) {
    ...
    Map<Long, Integer> moduleCountByTenantId = loadUserFacingModuleCounts(tenantIds);
  ...
  private List<TenantModule> getEnabledUserFacingModules(Long tenantId) {
    return tenantModuleRepository.findByTenantIdAndStatus(tenantId, "enabled").stream()
        .filter(tm -> ModuleCode.isProvisioningSelectableCode(tm.getModuleCode()))
        .toList();
  }

  private Map<Long, Integer> loadUserFacingModuleCounts(List<Long> tenantIds) {
    Map<Long, Integer> moduleCountByTenantId = new HashMap<>();
    tenantIds.forEach(tenantId -> moduleCountByTenantId.put(tenantId, 0));

    tenantModuleRepository.findByTenantIdInAndStatus(tenantIds, "enabled").stream()
        .filter(tm -> ModuleCode.isProvisioningSelectableCode(tm.getModuleCode()))
        .forEach(tm -> moduleCountByTenantId.merge(tm.getTenantId(), 1, Integer::sum));

    return moduleCountByTenantId;
  }
