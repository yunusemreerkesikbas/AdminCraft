...
public class RecaptchaServiceImpl implements RecaptchaService {
    ...
    @Value("${app.recaptcha.timeout-seconds:5}")
    ...
    public boolean verifyToken(String token, String action) {
        ...
        if (action != null) {
            String returnedAction = (String) responseBody.get("action");
            if (!action.equals(returnedAction)) {
                log.warn("reCAPTCHA action mismatch. Expected: {}, Got: {} (siteId: {})",
                        action, returnedAction, site.getId());
    ...
    public boolean isEnabled() {
        return siteRepository.findFirstByOrderByIdAsc()
                .map(site -> Boolean.TRUE.equals(site.getRecaptchaEnabled()))
                .orElse(false);
    }
