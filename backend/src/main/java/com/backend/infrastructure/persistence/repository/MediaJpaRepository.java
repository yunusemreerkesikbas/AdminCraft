...
public interface MediaJpaRepository extends JpaRepository<MediaFile, Long> {
       ...
       // Search queries - Updated to work with JSON-based i18n structure
       @Query("SELECT m FROM MediaFile m WHERE m.tenantId = :tenantId AND " +
                     "(LOWER(m.originalName) LIKE LOWER(CONCAT('%', :searchTerm, '%')) OR " +
                     "LOWER(m.fileName) LIKE LOWER(CONCAT('%', :searchTerm, '%')) OR " +
                     "LOWER(m.folder) LIKE LOWER(CONCAT('%', :searchTerm, '%')) OR " +
                     "LOWER(m.category) LIKE LOWER(CONCAT('%', :searchTerm, '%')) OR " +
                     "(m.i18n IS NOT NULL AND LOWER(m.i18n) LIKE LOWER(CONCAT('%', :searchTerm, '%'))))")
       ...
       List<MediaFile> findByTenantIdAndLastAccessedAtBetween(Long tenantId, LocalDateTime startDate,
       ...
       // Sprint 7: SHA-256 De-duplication queries
