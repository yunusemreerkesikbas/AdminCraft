...
interface ProductJpaRepository extends JpaRepository<Product, Long> {
    ...
    @EntityGraph(attributePaths = {"productType", "responsiveMediaSet", "i18nContent", "attributes", "categoryLinks", "gallery"})
    @Query("SELECT p FROM Product p WHERE p.id = :id")
    ...
    @Query(value = "SELECT DISTINCT p FROM Product p JOIN p.categoryLinks pcl WHERE pcl.category.id = :categoryId",
    ...
    @Query(value = "SELECT p FROM Product p WHERE p.status = 'PUBLISHED' AND p.isVisible = true",
    ...
    @EntityGraph(attributePaths = {"productType", "responsiveMediaSet", "i18nContent", "attributes", "attributes.attributeDefinition", "categoryLinks", "categoryLinks.category", "categoryLinks.category.i18nContent", "gallery", "gallery.media"})
    @Query("SELECT p FROM Product p WHERE p.id = :id")
