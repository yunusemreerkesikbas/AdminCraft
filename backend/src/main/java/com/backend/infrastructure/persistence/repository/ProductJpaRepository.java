...
interface ProductJpaRepository extends JpaRepository<Product, Long> {
    ...
    @Query(
        value = "SELECT DISTINCT p FROM Product p JOIN p.categoryLinks pcl WHERE pcl.category.id = :categoryId",
        countQuery = "SELECT COUNT(DISTINCT p) FROM Product p JOIN p.categoryLinks pcl WHERE pcl.category.id = :categoryId"
    )
    Page<Product> findByCategoryId(@Param("categoryId") Long categoryId, Pageable pageable);

    @Query(
        value = "SELECT DISTINCT p FROM Product p LEFT JOIN p.i18nContent i " +
            "WHERE LOWER(p.sku) LIKE LOWER(CONCAT('%', :query, '%')) " +
            "OR LOWER(i.name) LIKE LOWER(CONCAT('%', :query, '%'))",
        countQuery = "SELECT COUNT(DISTINCT p) FROM Product p LEFT JOIN p.i18nContent i " +
            "WHERE LOWER(p.sku) LIKE LOWER(CONCAT('%', :query, '%')) " +
            "OR LOWER(i.name) LIKE LOWER(CONCAT('%', :query, '%'))"
    )
    ...
    @Query(
        value = "SELECT DISTINCT p FROM Product p LEFT JOIN p.i18nContent i LEFT JOIN p.categoryLinks pcl " +
        ...
        countQuery = "SELECT COUNT(DISTINCT p) FROM Product p LEFT JOIN p.i18nContent i LEFT JOIN p.categoryLinks pcl " +
            "WHERE (:query IS NULL OR LOWER(p.sku) LIKE LOWER(CONCAT('%', :query, '%')) OR LOWER(i.name) LIKE LOWER(CONCAT('%', :query, '%'))) " +
            "AND (:categoryId IS NULL OR pcl.category.id = :categoryId) " +
            "AND (:status IS NULL OR p.status = :status)"
    )
    ...
    @Query(
        value = "SELECT DISTINCT p FROM Product p JOIN p.categoryLinks pcl " +
            "WHERE pcl.category.id = :categoryId AND p.status = 'PUBLISHED' AND p.isVisible = true",
        countQuery = "SELECT COUNT(DISTINCT p) FROM Product p JOIN p.categoryLinks pcl " +
            "WHERE pcl.category.id = :categoryId AND p.status = 'PUBLISHED' AND p.isVisible = true"
    )
    Page<Product> findPublishedByCategoryId(@Param("categoryId") Long categoryId, Pageable pageable);

    @Query(
        value = "SELECT p FROM Product p WHERE p.status = 'PUBLISHED' AND p.isVisible = true",
        countQuery = "SELECT COUNT(p) FROM Product p WHERE p.status = 'PUBLISHED' AND p.isVisible = true"
    )
    ...
    @EntityGraph(attributePaths = { "productType", "responsiveMediaSet", "attributes",
        "attributes.attributeDefinition", "i18nContent", "categoryLinks", "gallery" })
    @Query("SELECT p FROM Product p WHERE p.id = :id")
    ...
    Page<Product> searchWithStatusAndCategory(
        @Param("query") String query,
        @Param("status") ProductStatus status,
        @Param("categoryId") Long categoryId,
        Pageable pageable
