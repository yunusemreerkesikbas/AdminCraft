...
class DemoRequestDeduplicationTest {
    ...
    @Mock private PlatformDemoRequestRepository repository;
    @Mock private RecaptchaService recaptchaService;
    @Mock private PlatformMailMarketingService mailMarketingService;
    ...
    void duplicate_withinWindow_suppressedSilently() {
        ...
        when(repository.hasRecentSubmission(anyString(), anyString(), any())).thenReturn(true);
    ...
    void firstSubmission_savedAndMailSent() {
        when(recaptchaService.verifyToken(anyString(), anyString())).thenReturn(true);
        when(repository.hasRecentSubmission(anyString(), anyString(), any())).thenReturn(false);
        when(repository.save(any(PlatformDemoRequest.class))).thenAnswer(inv -> inv.getArgument(0));

        service.submit(command());

        verify(repository).save(any(PlatformDemoRequest.class));
    ...
    void differentIp_sameEmail_notDuplicate() {
        when(recaptchaService.verifyToken(anyString(), anyString())).thenReturn(true);
        when(repository.hasRecentSubmission(eq("user@example.com"), eq("1.2.3.5"), any())).thenReturn(false);
        when(repository.save(any(PlatformDemoRequest.class))).thenAnswer(inv -> inv.getArgument(0));
