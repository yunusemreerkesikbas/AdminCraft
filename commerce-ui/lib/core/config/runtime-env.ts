...
export const getCommerceBaseUrl = (): string => {
  const baseUrl = readFirstEnv(["NEXT_PUBLIC_COMMERCE_API_URL"]);
  if (!baseUrl) {
    throw new Error("NEXT_PUBLIC_COMMERCE_API_URL is required.");
  }
...
export const getTenantContext = (): TenantContext => {
  const tenantSubdomain = readFirstEnv([
    "TENANT_SUBDOMAIN",
    "NEXT_PUBLIC_TENANT_SUBDOMAIN",
  ]);
  ...
  throw new Error(
    "Tenant context is required. Set TENANT_SUBDOMAIN or TENANT_ID before starting commerce-ui.",
  );
};
