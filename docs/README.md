...
- Homepage body and shared chrome are CMS-driven via the default theme (`components/theme/default/`), `LandingPageTemplate` `Section1–8` slots, and shared `Header` / `Footer` slots
- Required tenant seed/import flow for the default landing page (ImpEx, manual via Admin UI `/{lang}/impex`):
  ...
  - ImpEx: `impex/seed_liko_components.sql` → `impex/seed_liko_chrome_components.sql` → `impex/seed_liko_pages_and_slots.sql` → `impex/seed_pages_and_slots.sql` → `impex/seed_navigation.sql`
