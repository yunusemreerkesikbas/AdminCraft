...
- **Deduplication (SEC-107):** if the same email + client IP submits again within 5 minutes, the request is silently discarded — no record is saved and no mail is sent. The response is still HTTP 200 with the standard success body (uniform payload to reduce enumeration risk; execution time may still vary).
