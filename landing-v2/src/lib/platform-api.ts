...
export async function getRecaptchaToken(siteKey: string): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!g) { reject(new Error("recaptcha_unavailable")); return; }
    g.ready(() => {
      g.execute(siteKey, { action: RECAPTCHA_ACTION }).then(resolve).catch(reject);
    });
  });
}
...
export class NewsletterSubscribeError extends Error {
  ...
  constructor(reason: "network" | "http", status: number, apiMessage: string | null) {
    super(apiMessage ?? reason);
...
export async function subscribeNewsletter(
  ...
  const origin = getCraftiveApiOrigin();
  if (!origin) throw new Error("api_not_configured");
  ...
  if (!body) throw new NewsletterSubscribeError("http", res.status, null);
  throw new NewsletterSubscribeError("http", res.status, body.message ?? null);
}
...
export async function submitDemoRequest(
  ...
  if (res.ok && body?.result === "SUCCESS") {
    ...
    if (rawData !== null && rawData !== undefined && typeof rawData === "object") {
      ...
      if (typeof v === "string") followUpNote = v.trim();
    }
    return { message, followUpNote };
  }

  if (!body) throw new DemoSubmitError("http", res.status, null);
  throw new DemoSubmitError("http", res.status, body.message ?? null);
}
