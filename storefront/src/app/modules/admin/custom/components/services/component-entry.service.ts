...
export class ComponentEntryService extends CrudHttpService<
    ComponentEntry,
    CreateEntryRequest,
    UpdateEntryRequest
> {
    ...
    override create(data: CreateEntryRequest): Observable<ComponentEntry> {
        return this.customPost<ComponentEntry>('componentEntriesCreate', data, {
    ...
    ): Observable<ComponentEntry> {
        return this.customPut<ComponentEntry>('componentEntryUpdate', data, {
            entryId,
        });
    }
    ...
    listByComponentId(componentId: number): Observable<ComponentEntry[]> {
        return this.customGet<ComponentEntry[]>('componentEntriesByComponent', {
            componentId,
        });
    }

    getEntryDetail(entryId: number): Observable<ComponentEntryDetailDto> {
        return this.customGet<ComponentEntryDetailDto>(
            'componentEntryById',
            { entryId },
    ...
    getI18n(entryId: number, language: string): Observable<EntryI18nDto> {
        return this.customGet<EntryI18nDto>('componentEntryI18n', {
            entryId,
            language,
        });
    }
    ...
    upsertI18n(
        entryId: number,
        language: string,
        data: EntryI18nRequest
    ): Observable<EntryI18nDto> {
        return this.customPut<EntryI18nDto>('componentEntryI18nUpsert', data, {
            entryId,
            language,
        });
    }
    ...
    publish(entryId: number, language: string): Observable<EntryI18nDto> {
        return this.customPost<EntryI18nDto>(
            'componentEntryPublish',
            {},
            { entryId, language }
        );
    }
