...
import { ProductCompositeRequest, ProductListItemResponse } from '../models/product.types';
...
export class ProductListComponent extends BasePaginatedListComponent<ProductListItemResponse, ProductCompositeRequest, ProductCompositeRequest> {
    protected override service = inject(ProductService) as unknown as CrudHttpService<ProductListItemResponse, ProductCompositeRequest, ProductCompositeRequest>;
    ...
    protected onGridAction(event: { action: string; item: ProductListItemResponse }): void {
    ...
    deleteProduct(item: ProductListItemResponse): void {
        this.#confirmation.confirm('admin.products.delete.title', 'admin.products.delete.message')
            .pipe(take(1))
            .subscribe(confirmed => {
                if (confirmed) {
    ...
    protected override onDeleteError(error: any): void {
        this.#notification.alert(error?.error?.message || 'admin.common.errors.deleteFailed');
    }
}
