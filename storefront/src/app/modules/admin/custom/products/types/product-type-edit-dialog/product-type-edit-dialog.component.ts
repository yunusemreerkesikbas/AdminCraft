...
export class ProductTypeEditDialogComponent
    extends SpaLocalizedFormDialog<boolean, ProductTypeEditDialogData>
    implements OnInit
{
    ...
    tabs = [
        ...
        {
            id: 'attributes',
            label: 'admin.products.types.tabs.attributes',
            icon: 'list_alt',
    ...
    override ngOnInit(): void {
        ...
        if (this.data.mode === 'edit' && this.data.item) {
            setTimeout(() => this.loadAttributes());
        }
    }
        ...
        return this.fb.group({
            code: [
                ...
                [
                    ...
                    Validators.maxLength(
                        VALIDATION_LIMITS.PRODUCT_TYPE_CODE_MAX
                    ),
            ...
            name: [
                ...
                [
                    Validators.required,
                    Validators.maxLength(
                        VALIDATION_LIMITS.PRODUCT_TYPE_NAME_MAX
                    ),
                ],
            ],
            category: [
                ...
                [
                    Validators.maxLength(
                        VALIDATION_LIMITS.PRODUCT_TYPE_CATEGORY_MAX
                    ),
                ],
            ],
            createdAt: [
                {
                    value: this.#formatDateTime(this.data.item?.createdAt),
                    disabled: true,
                },
            ],
            updatedAt: [
                {
                    value: this.#formatDateTime(this.data.item?.updatedAt),
                    disabled: true,
                },
            ],
        });
    ...
    loadAttributes(): void {
        ...
        this.#productTypeService
            .getAttributes(this.data.item.id)
            .pipe(take(1))
            .subscribe({
                next: (attrs) => {
                ...
                error: () => {
                    this.#notificationService.alert(
                        'admin.products.types.errors.loadAttributesFailed'
                    );
                    this.isLoadingAttributes.set(false);
                },
            });
    ...
    addAttribute(): void {
        this.#matDialog
            ...
            .subscribe((result) => {
                if (result) {
                    this.#productTypeService
                        .createAttribute(this.data.item!.id, result)
                        .pipe(take(1))
                        .subscribe({
                            next: () => {
                                this.#notificationService.success(
                                    'admin.products.attributes.createSuccess'
                                );
                                this.loadAttributes();
                            },
                            error: (err) =>
                                this.#notificationService.alert(
                                    err?.error?.message ||
                                        'admin.products.attributes.createFailed'
                                ),
                        });
                }
            });
        ...
        this.#matDialog
            ...
            .pipe(
                ...
                switchMap((result) => {
                    ...
                    // Check if fieldType is changing
                    const isFieldTypeChanging = result.fieldType !== attr.fieldType;

                    if (isFieldTypeChanging) {
                        // Get product count to show in warning
                        return this.#productTypeService
                            .getProductCount(this.data.item!.id)
                            .pipe(
                                take(1),
                                switchMap((count) => {
                                    if (count > 0) {
                                        ...
                                        return this.#confirmationService
                                            .confirm(
                                                'admin.products.attributes.fieldTypeChange.title',
                                                this.#translocoService.translate(
                                                    'admin.products.attributes.fieldTypeChange.message',
                                                    { count }
                                                )
                                            )
                                            .pipe(
            ...
            .subscribe((result) => {
                if (result) {
                    this.#productTypeService
                        .updateAttribute(this.data.item!.id, attr.id, result)
                        .pipe(take(1))
                        .subscribe({
                            next: () => {
                                this.#notificationService.success(
                                    'admin.products.attributes.updateSuccess'
                                );
                                this.loadAttributes();
                            },
                            error: (err) =>
                                this.#notificationService.alert(
                                    err?.error?.message ||
                                        'admin.products.attributes.updateFailed'
                                ),
                        });
                }
            });
    ...
    deleteAttribute(attr: AttributeDefinition): void {
        this.#confirmationService
            .confirm(
                'admin.products.attributes.delete.title',
                'admin.products.attributes.delete.message'
            )
            .pipe(take(1))
            .subscribe((confirmed) => {
                if (confirmed) {
                    this.#productTypeService
                        .deleteAttribute(this.data.item!.id, attr.id)
                        .pipe(take(1))
                        .subscribe({
                            next: () => {
                                this.#notificationService.success(
                                    'admin.products.attributes.deleteSuccess'
                                );
                                this.loadAttributes();
                            },
                            error: (err) =>
                                this.#notificationService.alert(
                                    err?.error?.message ||
                                        'admin.products.attributes.deleteFailed'
                                ),
                        });
                }
            });
    ...
    getAttributeTypeClass(type: string): string {
        switch (type) {
            case 'TEXT':
